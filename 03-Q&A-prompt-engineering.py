...
# MAGIC - 最後の分散 Spark 推論を実行しない場合、このノートブックの例を実行するに必要なのは GPU を備えた単一ノードの「クラスター」だけです
...
# MAGIC - 最後の分散 Spark 推論を使用してこの例を実行するには、複数GPU のクラスターをプロビジョニングします (そして、GPU 数に一致するように最後のリパーティションを変更します)。
# MAGIC
